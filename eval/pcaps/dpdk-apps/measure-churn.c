...
struct expired_t expired[32];
struct received_t received[32];

void signal_handler(int dummy) {
  uint64_t total_expired = 0;
  for (int i = 0; i < 32; i++) {
    total_expired += expired[i].value;
    printf("%d\t: %ld\t(%.2f %%)\n", i, expired[i].value,
           100.0 * expired[i].value / received[i].value);
  }
...
void null_init(void *obj) { *(uint32_t *)obj = 0; }
...
uint8_t hash_key_0[RSS_HASH_KEY_LENGTH] = {
    0x5c, 0x21, 0x41, 0xed, 0x93, 0xed, 0x7a, 0x64, 0x49, 0x64, 0xde,
    0x88, 0x55, 0x17, 0x12, 0x67, 0xf7, 0xe2, 0x10, 0x30, 0xcd, 0xbf,
    0x5c, 0x21, 0x41, 0xed, 0x93, 0xed, 0x7a, 0x64, 0x49, 0x64, 0xde,
    0x88, 0x55, 0x17, 0x12, 0x67, 0xf7, 0xe2, 0x10, 0x30, 0xcd, 0xbf,
    0xb9, 0xdc, 0x98, 0x3f, 0xe0, 0x5,  0x73, 0xd1};
...
bool nf_init() {
  ...
  int map_allocation_succeeded__1 =
      map_allocate(FlowId_eq, FlowId_hash, 65536u, &(*map_ptr));

  if (map_allocation_succeeded__1) {
    int vector_alloc_success__4 =
        vector_allocate(16u, 65536u, FlowId_allocate, &(*vector_ptr));

    if (vector_alloc_success__4) {
      int vector_alloc_success__7 =
          vector_allocate(4u, 65536u, null_init, &(*vector_1_ptr));

      if (vector_alloc_success__7) {
        int is_dchain_allocated__10 = dchain_allocate(65536u, &(*dchain_ptr));
...
int nf_process(uint16_t device, uint8_t *packet, uint16_t packet_length,
  ...
  uint64_t exp_time_us = 1000;

  int number_of_freed_flows__26 = expire_items_single_map(
      (*dchain_ptr), (*vector_ptr), (*map_ptr), now - exp_time_us * 1000);

  expired[rte_lcore_id()].value += number_of_freed_flows__26;
  received[rte_lcore_id()].value++;

  struct rte_ether_hdr *ether_header = (struct rte_ether_hdr *)(packet);

  if (8u == ether_header->ether_type) {
    struct rte_ipv4_hdr *ipv4_header = (struct rte_ipv4_hdr *)(packet + 14u);

    if ((ipv4_header->version_ihl & 15u) <= 5u) {
      if ((6u == ipv4_header->next_proto_id) ||
          (17u == ipv4_header->next_proto_id)) {
        struct tcpudp_hdr *tcpudp_header =
            (struct tcpudp_hdr *)(packet + (14u + 20u));

        if (device) {
          uint8_t map_key[13];
          map_key[0u] = tcpudp_header->dst_port & 0xff;
          map_key[1u] = (tcpudp_header->dst_port >> 8) & 0xff;
          map_key[2u] = tcpudp_header->src_port & 0xff;
          map_key[3u] = (tcpudp_header->src_port >> 8) & 0xff;
          map_key[4u] = ipv4_header->dst_addr & 0xff;
          map_key[5u] = (ipv4_header->dst_addr >> 8) & 0xff;
          map_key[6u] = (ipv4_header->dst_addr >> 16) & 0xff;
          map_key[7u] = (ipv4_header->dst_addr >> 24) & 0xff;
          map_key[8u] = ipv4_header->src_addr & 0xff;
          map_key[9u] = (ipv4_header->src_addr >> 8) & 0xff;
          map_key[10u] = (ipv4_header->src_addr >> 16) & 0xff;
          map_key[11u] = (ipv4_header->src_addr >> 24) & 0xff;
          map_key[12u] = ipv4_header->next_proto_id;
          int map_value_out;
          int map_has_this_key__42 =
              map_get((*map_ptr), &map_key, &map_value_out);
          ...
          if (0u == map_has_this_key__42) {
            ...
            vector_borrow((*vector_1_ptr), map_value_out,
                          (void **)(&vector_value_out));
          ...
          uint8_t map_key[13];
          map_key[0u] = tcpudp_header->src_port & 0xff;
          map_key[1u] = (tcpudp_header->src_port >> 8) & 0xff;
          map_key[2u] = tcpudp_header->dst_port & 0xff;
          map_key[3u] = (tcpudp_header->dst_port >> 8) & 0xff;
          map_key[4u] = ipv4_header->src_addr & 0xff;
          map_key[5u] = (ipv4_header->src_addr >> 8) & 0xff;
          map_key[6u] = (ipv4_header->src_addr >> 16) & 0xff;
          map_key[7u] = (ipv4_header->src_addr >> 24) & 0xff;
          map_key[8u] = ipv4_header->dst_addr & 0xff;
          map_key[9u] = (ipv4_header->dst_addr >> 8) & 0xff;
          map_key[10u] = (ipv4_header->dst_addr >> 16) & 0xff;
          map_key[11u] = (ipv4_header->dst_addr >> 24) & 0xff;
          map_key[12u] = ipv4_header->next_proto_id;
          int map_value_out;
          int map_has_this_key__62 =
              map_get((*map_ptr), &map_key, &map_value_out);

          if (0u == map_has_this_key__62) {
            uint32_t new_index__65;
            int out_of_space__65 =
                !dchain_allocate_new_index((*dchain_ptr), &new_index__65, now);

            if (false ==
                ((out_of_space__65) & (0u == number_of_freed_flows__26))) {
              uint8_t *vector_value_out = 0u;
              vector_borrow((*vector_ptr), new_index__65,
                            (void **)(&vector_value_out));
              vector_value_out[0u] = tcpudp_header->src_port & 0xff;
              vector_value_out[1u] = (tcpudp_header->src_port >> 8) & 0xff;
              vector_value_out[2u] = tcpudp_header->dst_port & 0xff;
              vector_value_out[3u] = (tcpudp_header->dst_port >> 8) & 0xff;
              vector_value_out[4u] = ipv4_header->src_addr & 0xff;
              vector_value_out[5u] = (ipv4_header->src_addr >> 8) & 0xff;
              vector_value_out[6u] = (ipv4_header->src_addr >> 16) & 0xff;
              vector_value_out[7u] = (ipv4_header->src_addr >> 24) & 0xff;
              vector_value_out[8u] = ipv4_header->dst_addr & 0xff;
              vector_value_out[9u] = (ipv4_header->dst_addr >> 8) & 0xff;
              vector_value_out[10u] = (ipv4_header->dst_addr >> 16) & 0xff;
              vector_value_out[11u] = (ipv4_header->dst_addr >> 24) & 0xff;
              vector_value_out[12u] = ipv4_header->next_proto_id;
              vector_value_out[13u] = 171u;
              vector_value_out[14u] = 171u;
              vector_value_out[15u] = 171u;
              map_put((*map_ptr), vector_value_out, new_index__65);
              vector_return((*vector_ptr), new_index__65, vector_value_out);
              uint8_t *vector_value_out_1 = 0u;
              vector_borrow((*vector_1_ptr), new_index__65,
                            (void **)(&vector_value_out_1));
              vector_value_out_1[0u] = 0u;
              vector_value_out_1[1u] = 0u;
              vector_value_out_1[2u] = 0u;
              vector_value_out_1[3u] = 0u;
              vector_return((*vector_1_ptr), new_index__65, vector_value_out_1);
              return 1;
            }

            else {
              return 1;
            }  // !(false == ((out_of_space__65) & (0u ==
               // number_of_freed_flows__26)))
          }

          else {
            dchain_rejuvenate_index((*dchain_ptr), map_value_out, now);
            return 1;
          }  // !(0u == map_has_this_key__62)
      ...
      // dropping
      return device;
    }  // !(0u == (((ipv4_header->version_ihl & 15u) < (5u)) | ((4294967282u +
       // packet_length) < (((ipv4_header->total_length & 255u) << 8u) |
       // ((ipv4_header->total_length & 65280u) >> 8u)))))
    ...
    // dropping
    return device;
  }  // !(0u == ((8u != ether_header->ether_type) | ((4294967282u +
     // packet_length) < 20ul)))
}
